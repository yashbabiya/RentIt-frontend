...
const searchProducts = async()=>{
  ...
  try{
    const res = await axios.get(API+`/product/search?keyword=${queryKeyword}&category=${categoriesValue}&page=1&limit=10`)
  ...
  return (
    <motion.div className="page explore"
    initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
  ...
  <i className="im im-filter"></i>
        ...
        <button className={`pill yellow`} onClick={()=>{
          fetchProducts()
}}>All</button>
          {options.map((op)=><button className={`pill ${category.indexOf(op.value) != -1 ? 'blue' : 'yellow'}`} onClick={()=>{
      ...
      <div className="listProduct">{
        ...
        <div className='flex-col'><img height={80} width={80}  src={emptyBox}/>No Products</div>
